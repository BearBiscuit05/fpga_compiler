...
public:
	void setEnv(){
        ...
        std::string binary_file = getBoardBinaryFile("/home/fpga/HGC/bellman/bin/bellman", this->device);
        printf("Using AOCX: %s\n", binary_file.c_str());
...
public:
	...
	Bellman(std::string GraphPath,int initNode,int partition,int iter){
        loadGraph(GraphPath);
...
int main(int argc, char **argv){
    ...
    Bellman bellman = Bellman(path,initNode[ID],2,iter[ID]);
    return 0;
}

...
public:
	void setEnv(){
        ...
        std::string binary_file = getBoardBinaryFile("/home/fpga/HGC/WCC/bin/wcc", this->device);
        printf("Using AOCX: %s\n", binary_file.c_str());
...
public:
	...
	WCC(std::string GraphPath,int initNode,int partition,int iter){
        loadGraph(GraphPath);
	...
	void MSGGenMerge_FPGA(Graph& g, std::vector<int>& mValue){
        ...
        size_t globalSize = g.eCount,dim = 1;
        cl_int iStatus = 0;
        int index = -1,kernelID = 0;

        if(env.clMem.size() == 0){
#ifdef MERGE_DEBUG
            std::cout<<"run one time"<<std::endl;
#endif
        ...
        cl_event read_event;
        iStatus = clEnqueueReadBuffer(env.queue, env.clMem[kernelID][4], CL_TRUE, 0, this->MemSpace * sizeof(int), &mValue[0], 0, NULL, &read_event);
...
int main(int argc, char *argv[]){
    ...
    int ID = argv[1][0] - '0';
    WCC wcc = WCC(filePath[ID],initNode[ID],2,iter[ID]);
    return 0;
}
